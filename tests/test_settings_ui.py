...
def test_settings_update_invalid_value_shows_error(client, app):
    ...
    with app.app_context():
        ...
        assert (
            b"Weekly Hours: Must be greater than 0 and at most 168 hours"
            in response.data
        )
...
def test_settings_validates_standard_hours_per_day_range(client, app):
    ...
    with app.app_context():
        ...
        assert response.status_code == 200
        assert (
            b"Standard Hours Per Day: Must be greater than 0 and at most 24 hours"
            in response.data
        )
...
def test_settings_validates_pause_duration_range(client, app):
    ...
    with app.app_context():
        ...
        assert response.status_code == 200
        assert (
            b"Pause Duration (minutes): Must be between 0 and 480 minutes (8 hours)"
            in response.data
        )
...
def test_settings_validates_max_work_session_hours_range(client, app):
    ...
    with app.app_context():
        ...
        assert response.status_code == 200
        assert (
            b"Max Work Session Hours: Must be between 1 and 24 hours" in response.data
        )
